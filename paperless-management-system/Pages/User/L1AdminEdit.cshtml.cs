...
{
    [Authorize(Roles = "L1 Admin")]
    public class L1AdminEditModel : UserManagementPageModel
    {
        ...
        public L1AdminEditModel(UserManager<ApplicationUser> mgr, RoleManager<IdentityRole> roleMgr, ApplicationDbContext context)
        {
            _userManager = mgr;
            ...
            if (updateUser != null)
            {
                if (this.user.UploadImage != null)
                {
                    if (ValidateFile(this.user.UploadImage))
                    {
                        ...
                        return RedirectToPage("./L1AdminPage");
                    }
                    else
                    {

...
var SelectComponent = /*#__PURE__*/function (_Field) {
  ...
  _createClass(SelectComponent, [{
    ...
    value: function itemTemplate(data, value) {
      ...
      var template = this.sanitize(this.getSelectTemplate(data, value));
        ...
        if (keys) {
          keys.forEach(function (key) {
            ...
            if (propKey && propKey.length > 1) {
              _this3.templateKeys.push(propKey[1]);
            }
          });
        }
      }
        ...
        case 'indexeddb':
          {
            ...
            if (this.component.indexeddb && this.component.indexeddb.database && this.component.indexeddb.table) {
              ...
              request.onupgradeneeded = function (event) {
                if (_this7.component.customOptions) {
                  var db = event.target.result;
                  var objectStore = db.createObjectStore(_this7.component.indexeddb.table, {
                    keyPath: 'myKey',
                    autoIncrement: true
                  });

                  objectStore.transaction.oncomplete = function () {
                    var transaction = db.transaction(_this7.component.indexeddb.table, 'readwrite');

                    _this7.component.customOptions.forEach(function (item) {
                      transaction.objectStore(_this7.component.indexeddb.table).put(item);
                    });
                  };
                }
              };
              ...
              request.onsuccess = function (event) {
                var db = event.target.result;
                var transaction = db.transaction(_this7.component.indexeddb.table, 'readwrite');
                var objectStore = transaction.objectStore(_this7.component.indexeddb.table);
                new _nativePromiseOnly.default(function (resolve) {
                  ...
                  if (!_lodash.default.isEmpty(_this7.component.indexeddb.filter)) {
                    items = _lodash.default.filter(items, _this7.component.indexeddb.filter);
                  }
    ...
    value: function attach(element) {
      ...
      if (this.component.searchField) {
        ...
        this.addEventListener(input, 'search', function (event) {
          return _this8.triggerUpdate(event.detail.value);
        });
      ...
      if (this.isSelectResource && this.refs.addResource) {
        this.addEventListener(this.refs.addResource, 'click', function (event) {
          ...
          var projectUrl = _lodash.default.get(_this8.root, 'formio.projectUrl', _Formio.default.getBaseUrl());
    ...
    value: function normalizeValue(value) {
      ...
      if (this.component.multiple && Array.isArray(value)) {
        return value.map(function (singleValue) {
          return _this11.normalizeSingleValue(singleValue);
        });
      }
    ...
    value: function setValue(value) {
      ...
      this.itemsLoaded.then(function () {
        _this12.addValueOptions();

        _this12.setChoicesValue(value, hasPreviousValue, flags);
      });
      return changed;
    }
  }, {
      ...
      if (values) {
        if (_lodash.default.isObject(value)) {
          var compareComplexValues = function compareComplexValues(optionValue) {
            var normalizedOptionValue = _this13.normalizeSingleValue(optionValue);
        ...
        return values.findIndex(function (optionValue) {
          return _this13.normalizeSingleValue(optionValue) === value;
        }) !== -1;
      }
    ...
    value: function getOptionsValues() {
      ...
      return rawItems.map(function (item) {
        return _this14.getOptionValue(_this14.itemValue(item));
      });
    }
    /**
    ...
    value: function getNormalizedValues() {
      ...
      return this.component.data.values.map(function (value) {
        return {
          label: value.label,
          value: String(_this15.normalizeSingleValue(value.value))
        };
      });
    }
  }, {
    ...
    value: function asString(value) {
      ...
      var convertToString = function convertToString(data, valueProperty) {
        ...
        if (_this16.isBooleanOrNumber(data)) {
          data = data.toString();
        }

        if (Array.isArray(data) && data.some(function (item) {
          return _this16.isBooleanOrNumber(item);
        })) {
          data = data.map(function (item) {
            if (_this16.isBooleanOrNumber(item)) {
              item = item.toString();
            }
          });
        }
      ...
      if (['values', 'custom'].includes(this.component.dataSrc) && !this.asyncCustomValues()) {
        ...
        var getFromValues = function getFromValues() {
          ...
          var values = _this16.defaultSchema.data.values || [];
      ...
      if (Array.isArray(value)) {
        ...
        value.forEach(function (item) {
          return _items.push(_this16.itemTemplate(item));
        });

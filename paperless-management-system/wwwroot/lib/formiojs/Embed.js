...
function embed() {
  ...
  if (thisScript) {
    ...
    debug('Embeding Configuration', config); // The id for this embedded form.

    var id = "formio-".concat(Math.random().toString(36).substring(7));
    config.id = id;
    debug('Creating form wrapper');
    document.write("<div id=\"".concat(id, "-wrapper\"></div>"));
    var wrapper = document.getElementById("".concat(id, "-wrapper")); // If we include the libraries, then we will attempt to run this in shadow dom.
